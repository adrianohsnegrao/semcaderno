...
Cycle 023 selects strict normalized-email/password input and an application-owned verification boundary. The infrastructure verifier stores only Argon2id PHC strings with unique salts and performs equivalent hash work for unknown identities. Wrong password, unknown identity, disabled User, and absent credential binding share the same public `AUTHENTICATION_FAILED` result; correct proof is required before verification-required guidance. The browser-safe ID04 request schema necessarily validates the transient password field, but password material is never present in responses, logs, persistence, audit, support, analytics, or telemetry; hashes and verifier details never enter browser-safe contracts.
...
Cycle 026 implements only the pre-session challenge persistence boundary. Canonical raw `p1` evidence is parsed and HMAC-digested at the server edge; application and PostgreSQL receive only the purpose-branded versioned digest. PostgreSQL stores no identity or request metadata and accepts one consumption only while `created_at <= consumedAt < expires_at`. Unknown, expired, and replayed evidence share one negative result; database and decoding failures remain failures. This atomic row consumption proves persistence-layer replay resistance but does not claim HTTP origin validation, full CSRF enforcement, or an atomic sign-in transaction.

Cycle 027 selects a fixed-start aggregate window rather than retaining attempt timestamps to simulate an exact sliding window. The server derives one pseudonymous version-1 account key as HMAC-SHA-256 over UTF-8 `sem-caderno/sign-in-rate-limit/v1`, one zero byte, and the accepted normalized-email UTF-8 bytes. Only the complete 32-byte digest, fixed window bounds, saturated count, latest state-changing update, exact 24-hour retention deadline, and positive version may persist. The tenth admitted invalid proof closes later checks until the half-open window end; blocked checks do not extend retention. Rows are logically inactive at window-end equality and must be physically deleted by retention-expiry equality.

This key remains security-sensitive and server-side even though it is not accepted as authentication evidence. There is no raw/normalized email column, User link, attempt ledger, password-derived state, IP, agent, device, request history, analytics, or telemetry. HMAC, database, decoding, cleanup, and out-of-order-time failures remain internal failures and cannot fail open or mutate the aggregate silently.

Cycle 028 makes that boundary executable. Server code reuses the accepted normalized-email type and version-1 session HMAC key to derive one complete canonical account digest; application and persistence never receive the key or email. PostgreSQL stores only the digest and aggregate lifecycle, while per-key serialization prevents lost increments, threshold overflow, and ambiguous clear/record races. Saturated records do not extend retention, equality with window end permits verification, and bounded cleanup deletes only rows at or beyond exact retention expiry. Invalid key/time input, temporal regression, PostgreSQL failure, and cleanup failure reject rather than return an allowed decision.

Cycle 029 makes fresh session and authenticated-CSRF evidence generation executable only at the server security edge. Two independent Node CSPRNG calls produce 32 bytes each; `v1` and `c1` evidence remains transient, is not logged or persisted, and is immediately reduced to separately branded full HMAC-SHA-256 digests for inward use. The server-held key is validated, copied at construction, and absent from application, contracts, persistence, and public exports. CSPRNG, configuration, and derivation failures reject and cannot become issued evidence or anonymous success.

Cycle 030 freezes fail-closed issuance semantics. The transaction receives only the digest-only rate account key and purpose-separated session/CSRF digests. Unknown, expired, and consumed pre-session challenges share one rolled-back rejection; unusable locked User state shares one generic rolled-back rejection; infrastructure and rollback failure reject. Only structured named uniqueness conflicts for the new session or authenticated-CSRF digest produce an internal retry signal, and the server evidence owner discards both raw values before each of at most three total attempts.

Historical sessions receive no fabricated authenticated-CSRF evidence. A nullable all-or-complete digest pair preserves existing read-only inspection while missing binding rejects unsafe authenticated operations and token replenishment. Newly issued sessions require the complete pair. Mandatory success audit stores only User reference, fixed success codes, and the explicit issuance instant; it stores no session reference, rate key, email, password, bearer, digest, key, Business, IP, agent, device, request evidence, correlation, or metadata and is not a login-history facility.

Cycle 031 makes this boundary executable without expanding collection. PostgreSQL receives only complete purpose-specific digests, User identity, the account-rate digest, and explicit lifecycle instants. One transaction either commits challenge consumption, prior-session replacement, the new session/CSRF binding, minimal audit evidence, and rate clear together or preserves the prior state. The adapter logs no input or failure detail, exposes no raw evidence or constraint identity, and treats all non-authorized collision and infrastructure ambiguity as failure.

Cycle 032 composes those boundaries only at the server security edge. Rate checking precedes password verification; only the verifier's generic `invalid` result records aggregate failure, while verification-required, limited, challenge-rejected, User-rejected, crypto, and infrastructure paths do not create extra attempt history. Only `digestCollision` retries, each retry discards both raw values, and three total collisions fail closed. Raw successful evidence is returned from the internal operation only after persistence reports the exact committed User and expiry; no digest, key, password, normalized email, PostgreSQL identity, log, or telemetry field is added.
...
Cycle 024 makes only these inner boundaries executable. Contracts expose strict ID00/ID04 JSON schemas and the browser-readable `p1`/`c1` values required by the accepted flow; they export no session credential, digest, HMAC key, password hash, or verifier schema. Application receives normalized email/password proof through the verifier port and receives only versioned digest objects plus explicit dates through the issuance port. Verifier and issuance failures remain rejected promises and cannot become invalid proof or issued-session success implicitly.

Cycle 025 confines Argon2id and stored PHC verifiers to PostgreSQL infrastructure. The adapter accepts the transient password through the application-owned port, performs one real-or-dummy verifier operation, and neither logs nor persists that raw value. Unknown email, missing credential, and disabled User use one fixed non-secret dummy PHC; wrong proof remains invalid, while malformed PHC, Argon runtime, row decoding, query, and connection failures reject with fixed internal messages. This reduces trivial account-existence differences without claiming constant-time behavior.

The credential table stores only the User association, Argon2id PHC verifier, timestamps, and positive version. It contains no raw password, provider subject, recovery value, session/CSRF evidence, IP address, user agent, device/fingerprint/location data, login/request history, arbitrary metadata, analytics, telemetry, or authorization cache. Hashes remain server-only and are absent from browser-safe exports, public errors, and application results.
