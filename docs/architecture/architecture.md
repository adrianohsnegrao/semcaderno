...
> **Current implementation note:** the recruiter-testable release is governed by the
> [MVP Release Implementation Specification](../specs/mvp-release-implementation.md). Its
> catalog-only sales, normalized WhatsApp uniqueness and simple stock rules supersede the earlier
> provisional ad hoc-item, non-unique-contact and no-inventory choices in planning documents.
...
- The executable operational journey requires an active catalog Product, resolves price on the
  server, and preserves that name/price as the Sale Item snapshot.
...
Cycle 028 implements the Cycle 027 rate-limit authority. Application owns the purpose-specific `SignInRateLimitPort`; server derives its versioned key from accepted normalized email with HMAC-SHA-256 over the fixed rate domain, one zero byte, and normalized-email UTF-8 bytes; PostgreSQL receives only the complete digest and explicit operation time. `PostgresSignInRateLimitAdapter` serializes check, record, and clear through one transaction-scoped per-key advisory lock and row lock, so absent creation, increments, saturation, expiry replacement, and clear are linearizable without a lock table or generic transaction abstraction.

The sixth migration adds one `sign_in_rate_limits` aggregate keyed by version plus 32-byte digest. It stores only fixed window bounds, count capped at 10, latest state-changing update, exact 24-hour retention deadline, and positive version, with one retention-cleanup index. The adapter also exposes bounded infrastructure cleanup; scheduling remains deployment work. No attempt ledger, raw identity, User relation, HTTP orchestration, session issuance, or authorization enters this boundary.

Cycle 029 adds one server-owned evidence generator. It performs two separate Node `crypto.randomBytes(32)` calls, emits only canonical `v1` session and `c1` authenticated-CSRF evidence at the server edge, and derives the existing purpose-branded application digests with the accepted session-lookup and session-CSRF HMAC domains. The construction-owned key is copied and never exported inward. An optional byte-source collaborator exists only to make the cryptographic boundary deterministic under test; no generic token framework, application CSPRNG port, persistence, or HTTP orchestration is introduced.

Cycle 030 closes the atomic issuance authority without implementing it. The application input now carries the purpose-branded rate account key alongside User, explicit lifecycle instants, and three purpose-separated digests; its closed result separates committed issuance, locked-User rejection, uniform challenge rejection, and the two authorized digest-collision classes from rejected infrastructure failure. Persistence will reuse the Cycle 028 per-account transaction lock, then revalidate User, consume challenge, revoke only the presented active session, insert session plus authenticated-CSRF binding, append one minimal success audit row, and clear rate state in one transaction.

The compatibility profile is additive: historical sessions keep a null authenticated-CSRF pair and remain readable but cannot authorize unsafe operations or acquire a replacement token. New issuance requires a complete version-1 32-byte pair under partial uniqueness. Raw bearer values, normalized identity, HMAC keys, SQL errors, and constraint names do not enter application or browser-safe contracts; the server evidence owner alone handles the typed collision retry with three total attempts.

Cycle 031 implements that persistence boundary. `PostgresSessionIssuanceAdapter` acquires the existing rate-key advisory lock and performs User revalidation, guarded challenge consumption, exact prior-session revocation, complete session/CSRF insertion, minimal audit insertion, and rate-row deletion in one PostgreSQL transaction. Only the two named digest uniqueness identities become `digestCollision`; expected User/challenge outcomes are returned only after rollback, and every other persistence or rollback failure rejects.

Cycle 032 adds one internal `apps/server` sign-in coordinator. It accepts transport-validated email/password values, purpose-branded pre-session and optional prior-session digests, and one explicit issuance instant. The server normalizes email, derives the account key from its copied HMAC configuration, checks the aggregate before Argon2, records only `invalid`, and leaves clearing to the atomic Cycle 031 transaction. Verified proof triggers independent evidence generation and one initial issuance plus at most two complete retries; only `digestCollision` regenerates both bearers. User revalidation maps to generic authentication failure, challenge rejection remains distinct, and every malformed dependency result or infrastructure failure rejects.

The coordinator returns raw newly issued evidence only after an exact committed User/expiry result. It exports no Fastify route, cookie writer, PostgreSQL detail, digest, selected Business, Membership, authorization fact, logger, or hidden clock. ID00/ID04 HTTP exposure remains a later executable boundary.
