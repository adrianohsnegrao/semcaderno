...
## 38. Cycle 025 Password Credential Implementation

Cycle 025 adds the ordered `20260806000100-create-user-password-credentials` migration. `sem_caderno.user_password_credentials` uses `user_id uuid` as both primary key and restrictive foreign key to `users`, plus non-null `password_verifier text`, `created_at timestamptz`, `updated_at timestamptz`, and positive `version bigint`. Checks require an Argon2id prefix and nondecreasing update time. No independent identifier or index is needed because primary-email lookup uses the existing unique `users.email_normalized` key and the User primary key owns the one credential row.

`PostgresPasswordVerificationAdapter` executes one parameterized User/credential left join, selects only User identity/lifecycle/verification evidence and the verifier, validates the accepted Argon2id PHC minimums, and maps directly to the application-owned outcomes. Unknown, disabled, and missing-credential cases perform fixed non-secret dummy Argon2id work before returning invalid. Database, row, PHC, or verifier failure remains failure. No raw password, credential history, provider data, session/CSRF change, Business/Membership join, or authorization evidence is stored or queried.

## 39. Cycle 026 Pre-Session Challenge Implementation

The ordered `20260806000200-create-pre-session-challenges` migration creates `sem_caderno.pre_session_challenges` with UUIDv7 `id`, supported `digest_version`, 32-byte `challenge_digest`, `created_at`, `expires_at`, nullable `consumed_at`, and positive `version`. Named constraints enforce version 1, exact digest length, unique version/digest identity, expiry after creation, and any consumption inside the half-open lifetime interval. The unique constraint supplies the only lookup index required by current creation/consumption behavior; no cleanup index is added before a cleanup operation exists.
...
## 41. Cycle 028 Rate-Limit Persistence Implementation

The ordered `20260806000300-create-sign-in-rate-limits` migration implements `sem_caderno.sign_in_rate_limits` with the eight authorized fields and no surrogate identifier. The compound primary key is account-key version plus digest. Named checks enforce version 1, 32 digest bytes, exact 15-minute window duration, count 1 through 10, latest update inside the half-open window, exact 24-hour retention from that update, and positive row version. One `retention_expires_at` index supports bounded cleanup; the primary key supports account lookup and uniqueness.

`PostgresSignInRateLimitAdapter` decodes only canonical application account keys and uses one transaction-scoped advisory lock derived from digest bytes before locking or creating the keyed row. Advisory-key collisions can only serialize unrelated accounts; they cannot merge state because every query still uses the complete versioned digest primary key. Check is non-mutating, record creates/increments/saturates/replaces according to explicit time, and clear is idempotent. Saturated record calls issue no row update. Temporal regression and persistence faults reject through one fixed internal boundary.

The concrete adapter's bounded cleanup selects at most the supplied positive limit of `retention_expires_at <= evaluatedAt` rows with row locks and deletes them. It reads no implicit clock and introduces no scheduler. Real PostgreSQL tests prove exact schema and checks, first/tenth/saturated/expired transitions, retention equality, bounded cleanup, no lost concurrent increments, atomic expired replacement, and both forced clear/record linearization orders. No raw or normalized email, User relation, attempt row, password evidence, request metadata, or authorization state is persisted.

## 42. Cycle 030 Session Issuance Persistence Authority Closure

The next ordered migration is `20260806000400-add-session-issuance-foundation`. It adds `authenticated_csrf_digest_version smallint` and `authenticated_csrf_digest bytea` to `sem_caderno.sessions`, initially nullable with no default or backfill. Named checks require both columns to be null or both non-null, require version 1 when present, and require exactly 32 digest bytes when present. A named partial unique index over version plus digest applies only to complete pairs. The existing session credential uniqueness remains separate and unchanged.

Historical sessions remain null-pair rows. They may continue current-session inspection and read-only authenticated use until ordinary expiry or revocation, but cannot authorize unsafe operations or authenticated-CSRF replenishment. Every new issuance writer supplies a complete pair. A later contract migration may set both columns non-null only after legacy null pairs no longer exist and all writers enforce the pair; it must not fabricate or backfill authenticated-CSRF evidence.

The same migration creates the first minimal `sem_caderno.audit_events` profile with `id uuid` primary key defaulting to PostgreSQL `uuidv7()`, `actor_user_id uuid NOT NULL` with restrictive User foreign key, `action_code text NOT NULL`, `outcome_code text NOT NULL`, and `occurred_at timestamptz NOT NULL`. Named checks allow only `action_code = 'session_issued'` and `outcome_code = 'succeeded'` in this profile. No session reference, Business, correlation, reason, metadata, email, password, bearer, digest, key, IP, agent, device, or request evidence is stored. The issuance adapter inserts one row and exposes no update/delete operation; broader audit categories and retention remain future additive authority.

`IssueSessionInput` includes the complete versioned `SignInRateLimitAccountKey`. Persistence decodes it exactly as Cycle 028 and acquires the same transaction-scoped advisory lock from the first signed big-endian 64 digest bits before any User/challenge/session lock. It then locks and revalidates the User, consumes the challenge, conditionally revokes the exact active prior-session digest, inserts the complete new session/CSRF row, inserts the audit row, deletes the complete account-key aggregate, and commits. Zero matching prior active session and zero matching rate row are idempotent no-ops. Other device sessions are untouched.

The transaction returns only `issued`, `userRejected`, `preSessionChallengeRejected`, or `digestCollision`. Unknown, expired-at-equality, and consumed challenges share `preSessionChallengeRejected`; missing, disabled, and no-longer-verified locked Users share `userRejected`. These expected outcomes are returned only after successful rollback. Named session-credential or authenticated-CSRF unique conflicts alone become `digestCollision` after rollback, identified by structured PostgreSQL code and constraint identity rather than message text. Every other database, constraint, decoding, temporal, transaction, audit, or rollback fault rejects as infrastructure failure.

The server evidence owner retries only `digestCollision`, discarding and regenerating both raw values for at most three total transaction attempts. The adapter owns no CSPRNG or retry loop. Challenge/User rejection is not retried. Session expiry is exactly `issued_at + 12 hours`, audit occurrence equals `issued_at`, and no SQL current-time expression owns lifecycle state. Transaction rollback preserves challenge availability, prior-session state, absence of the new session/audit row, and the pre-existing rate aggregate.

## 43. Cycle 031 Session Issuance Persistence Implementation

The ordered `20260806000400-add-session-issuance-foundation` migration implements the nullable all-or-complete authenticated-CSRF pair, supported version and exact-length checks, and the named partial uniqueness index without default or backfill. It also creates only the five-column `audit_events` profile with restrictive User ownership and fixed `session_issued`/`succeeded` checks. Historical null-pair sessions remain valid for the existing read-only resolver; every new issuance row supplies a complete pair.

`PostgresSessionIssuanceAdapter` validates all versioned digests and the exact 12-hour lifecycle before opening a transaction. It acquires the Cycle 028 account advisory lock, revalidates the User, guarded-consumes the challenge, conditionally revokes the exact prior active digest, inserts the new session/CSRF pair and audit row, clears the exact rate aggregate, and commits. Expected User/challenge results and the two named collision results return only after rollback; every other PostgreSQL, constraint, mapping, temporal, transaction, or rollback failure rejects through one fixed internal boundary.

Real PostgreSQL 18.4 tests prove migration-from-six and repeated execution, historical compatibility, schema constraints, full commit state, rollback after expected and late failures, exact collision classification, no-op prior/rate absence, one-winner challenge issuance, and both forced rate-record/issuance-clear orders. Persistence receives no raw bearer, HMAC key, normalized identity, password, IP, agent, device, request history, or generic metadata. The server collision-retry coordinator, unsafe-operation CSRF enforcement, HTTP, and cookies remain outside this implementation.
