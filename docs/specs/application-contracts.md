...
Cycle 027 specifies the future `SignInRateLimitPort` without implementing it. Its purpose-branded account key contains only digest version 1 and canonical digest text. `check(accountKey, evaluatedAt)` returns allowed or limited with the exact retry instant; `recordFailure(accountKey, occurredAt)` returns the post-record state for subsequent attempts; and idempotent `clear(accountKey)` removes state only inside fully committed issuance. Counts remain persistence-internal. Only password-verification outcome `invalid` records a failure; verified, verification-required, malformed/CSRF-rejected/rate-rejected requests, and infrastructure failures do not. Port failures reject and cannot become allowed, limited, or invalid proof.
