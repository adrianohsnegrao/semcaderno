...
Cycle 030 closes the later issuance mapper inputs without implementing ID04 HTTP. A locked unusable User result maps to the existing generic 401 `AUTHENTICATION_FAILED`; unknown, expired-at-equality, or consumed pre-session evidence maps uniformly to 403 `CSRF_REJECTED`. The internal digest-collision result is never public: the server evidence owner retries with both fresh values for at most three total attempts, then maps exhaustion to safe 500 `INTERNAL_FAILURE`. Rejected persistence, transaction, rollback, audit, decoding, or unrelated constraint failure also remains `INTERNAL_FAILURE`. None of these failures writes a cookie or returns authenticated-CSRF evidence.
