...
> **Current implementation note:** automated coverage now includes Fastify routes, the in-memory
> demonstration adapter, PostgreSQL/Testcontainers persistence, session security, idempotency,
> duplicate contacts/products, server-authoritative prices, stock decrement and cancellation
> restoration. The [release specification](../specs/mvp-release-implementation.md) is the source of
> truth when older cycle-planning examples conflict with executable behavior.
